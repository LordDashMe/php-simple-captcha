...
| ```getCode();``` | To get the current code generated by the ```code(length);``` method. Return value ```QwErTyx...``` |
| ```getImage();``` | To get the current image generated by the ```image();``` method. Return value ```data:image/png;base64,iVBORw0KGgoAA...``` |
...
| ```getSession();``` | Use to get the current stored session generated values in the captcha session. This is use to validate the generated code against the user organic inputed code. <br> Return value ```array('code' => '...')``` |
